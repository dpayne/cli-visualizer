    ...
    vis::NcursesWriter *writer)
{
    ...
    //TODO: add const everywhere and use min of half height or half width
    const auto max_distance = std::sqrt(std::pow(half_height, 2) + std::pow(half_height, 2));
    ...
    for (auto i = 0u; i < bars.size(); ++i)
    {
        ...
        {
            auto x =
                static_cast<int32_t>(std::cos(rad) * row_index);
            ...
            const ColorIndex color_distance =
                writer->to_color_pair(static_cast<int32_t>(distance), static_cast<int32_t>(max_distance));

            x += half_height;
            y += half_width;


            //TODO: if stereo just use 180 degrees
            if (flipped)
            {
